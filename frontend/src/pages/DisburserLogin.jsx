...
import './disburser.css'
import heroBg from '../../image/img.jpg'
import { loginDisburser } from '../api/auth'

export default function DisburserLogin() {
	...
	const handleSubmit = async (e) => {
		...
		try {
			const data = await loginDisburser({
				email: form.email.value,
				password: form.password.value,
			})

			localStorage.setItem('disburser_token', data.token)
			localStorage.setItem('disburser_id', String(data.disburser_id))
			window.location.hash = '#/disburser-dashboard'
		} catch (err) {
	...
	return (
		<main className="disburser-page">
			<img className="disburser-bg" src={heroBg} alt="Office background" />
			<div className="disburser-overlay" aria-hidden="true" />

			<div className="disburser-container">
				<div className="disburser-card">
					<header className="disburser-header">
						<div className="disburser-badge">
							<svg viewBox="0 0 24 24" aria-hidden="true">
								<path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-6h2v6zm0-8h-2V7h2v2z" />
							</svg>
						</div>
						<h1>Disbursement Officer</h1>
						<p>Loan disbursement &amp; transaction portal</p>
					</header>

					<form className="disburser-form" onSubmit={handleSubmit}>
